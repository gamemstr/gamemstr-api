...
impl super::Model for Creature {
    ...
    fn to_entity(&self) -> Self::Entity {
        Self::Entity::new(
            self.id.clone(),
            self.name.clone(),
            serde_json::from_value(self.creature_type.clone()).unwrap(),
            serde_json::from_value(self.alignment.clone()).unwrap(),
            self.armor_class,
            serde_json::from_value(self.health_points.clone()).unwrap(),
            serde_json::from_value(self.speed.clone()).unwrap(),
            serde_json::from_value(self.stats.clone()).unwrap(),
            serde_json::from_value(self.saving_throws.clone().unwrap()).unwrap(),
            serde_json::from_value(self.damage_resistances.clone().unwrap()).unwrap(),
            serde_json::from_value(self.damage_immunities.clone().unwrap()).unwrap(),
            serde_json::from_value(self.damage_vulnerabilities.clone().unwrap()).unwrap(),
            serde_json::from_value(self.condition_immunities.clone().unwrap()).unwrap(),
            serde_json::from_value(self.skills.clone().unwrap()).unwrap(),
            serde_json::from_value(self.senses.clone().unwrap()).unwrap(),
            serde_json::from_value(self.languages.clone().unwrap()).unwrap(),
            serde_json::from_value(self.challenge_rating.clone()).unwrap(),
            serde_json::from_value(self.racial_traits.clone().unwrap()).unwrap(),
            self.description.clone(),
            serde_json::from_value(self.actions.clone().unwrap()).unwrap(),
            serde_json::from_value(self.lair.clone().unwrap()).unwrap(),
            serde_json::from_value(self.others.clone().unwrap()).unwrap(),
        )
    }
}
